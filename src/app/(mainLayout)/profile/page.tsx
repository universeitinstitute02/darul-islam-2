...
export default function ProfilePage() {
  ...
  const { data: userProfile, isLoading } = useUser(); // <-- Global Cache Hook
  const [activeTab, setActiveTab] = useState("dashboard");

  const userData = {
    name: userProfile?.name || session?.user?.name || "লোড হচ্ছে...",
    email: userProfile?.email || session?.user?.email || "তথ্য নেই",
    phone: userProfile?.phone || "তথ্য নেই",
    role: userProfile?.role === "teacher" ? "শিক্ষক" : "শিক্ষার্থী",
    joinDate: userProfile?.createdAt
      ? new Date(userProfile.createdAt).toLocaleDateString("bn-BD", {
          day: "numeric",
          month: "long",
          year: "numeric",
        })
          ...
          <div className="flex items-center gap-6">
            ...
            <div>
              ...
              <p className="text-xs lg:text-sm font-bold text-[#E2D4B9]/80 uppercase tracking-widest mt-1 flex items-center gap-2">
                <University size={14} /> {userData.role} · দারুল ইসলাম
                ইনস্টিটিউট
              </p>
            </div>
          </div>
        ...
        <aside className="lg:col-span-3 space-y-4">
          <div className="bg-white p-4 rounded-[2rem] shadow-xl border border-[#14281D]/5 space-y-1">
            ...
            <button
              ...
              className="w-full flex items-center gap-3 p-4 rounded-2xl font-black text-red-500 hover:bg-red-50 transition-all"
            >
              <LogOut size={20} />
              <span>লগআউট</span>
            </button>
          ...
          <div className="bg-[#14281D] p-6 rounded-[2rem] text-[#E2D4B9] shadow-2xl space-y-4">
            ...
            <h4 className="text-lg font-black leading-tight">
              আপনার কি কোনো সহযোগিতা প্রয়োজন?
            </h4>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
            ...
            <div className="lg:col-span-2 space-y-8">
              <section className="bg-[#14281D] p-8 rounded-[3rem] shadow-2xl text-[#E2D4B9] space-y-6 relative overflow-hidden">
                ...
                {isLoading ? (
                  <p className="text-sm font-bold relative z-10 text-white/60">
                    তথ্য লোড হচ্ছে...
                  </p>
                ) : (
                  <div className="space-y-4 relative z-10">
                    ...
                    <div className="flex items-center gap-4 p-4 bg-white/5 rounded-2xl border border-white/10">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-bold">{userData.joinDate}</p>
                      </div>
                    </div>
                  </div>
                )}

                <button className="w-full bg-[#E2D4B9] text-[#14281D] py-4 rounded-2xl font-black text-sm relative z-10 hover:bg-white transition-colors">
                  এডিট প্রোফাইল
                </button>
              </section>
